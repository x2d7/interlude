...
func TestComplete_SuccessToolCall(t *testing.T) {
	...
	mockClient := NewMockClient()
	mockClient.SetStreamingEvents([]StreamEvent{
		NewEventToken("Calling tool"),
		NewEventToolCall("call-123", "weather", `{"city": "Moscow"}`),
	})
...
func TestSession_Refusal(t *testing.T) {
	...
	// Note: EventCompletionEnded is generated by Session, not the mock
	mockClient.SetStreamingEvents([]StreamEvent{
		NewEventRefusal("I cannot help with that"),
	})
...
func TestSession_NonExistentTool(t *testing.T) {
	...
	// Round 1: tool call, Round 2: empty to finish (Session generates CompletionEnded)
	mockClient := NewMultiRoundMockClient([][]StreamEvent{
		{NewEventToolCall("call-1", "non-existent", `{}`)},
		{},
	})
...
func TestSession_ContextCancelledWhileWaitingForApproval(t *testing.T) {
	...
	var toolCallEvent EventToolCall
	for event := range result {
		if tc, ok := event.(EventToolCall); ok {
			toolCallEvent = tc
			break
...
func TestSession_ToolCallAssembly_Basic(t *testing.T) {
	...
	mockClient := NewMultiRoundMockClient([][]StreamEvent{
		{
			NewEventToolCall("call-1", "weather", `{"city": "`),
			NewEventToolCall("", "weather", "Moscow"),
			NewEventToolCall("", "weather", `"}`),
		},
	...
	messages := chat.Messages.Snapshot()
	var toolCall EventToolCall
	for _, msg := range messages {
		if tc, ok := msg.(EventToolCall); ok {
			toolCall = tc
			break
		}
	}
...
func TestSession_ToolCallAssembly_MultipleToolsWithAssembly(t *testing.T) {
	...
	mockClient := NewMultiRoundMockClient([][]StreamEvent{
		{
			NewEventToolCall("call-1", "tool1", `{"param": `),
			NewEventToolCall("", "tool1", "123"),
			NewEventToolCall("", "tool1", "}"),
			NewEventToolCall("call-2", "tool2", `{"val": `),
			NewEventToolCall("", "tool2", "456"),
			NewEventToolCall("", "tool2", "}"),
			NewEventToolCall("call-3", "tool2", `{"value": 789}`),
		},
		// Second round: empty (session exits cleanly)
		{},
	})
	...
	messages := chat.Messages.Snapshot()
	toolCalls := make([]EventToolCall, 0)
	for _, msg := range messages {
		if tc, ok := msg.(EventToolCall); ok {
...
func TestSession_ToolCallAssembly_LargeContent(t *testing.T) {
	...
	// Session generates CompletionEnded internally, so we don't add it to mocks
	events := []StreamEvent{
		NewEventToolCall("call-1", "search", `["chunk0"`),
	}
	// Add more chunks without CallID
	for i := 1; i < 20; i++ {
		chunk := fmt.Sprintf(",\"chunk%d\"", i)
		events = append(events, NewEventToolCall("", "search", chunk))
	}
	events = append(events, NewEventToolCall("", "search", `]`))
...
func TestSession_MixedTokensAndToolCalls(t *testing.T) {
	...
	mockClient := NewMultiRoundMockClient([][]StreamEvent{
		{
			NewEventToken("A"),
			NewEventToolCall("call-1", "lookup", `{"q":"1"}`),
			NewEventToken("B"),
			NewEventToolCall("call-2", "lookup", `{"q":"2"}`),
			NewEventToken("C"),
		},
